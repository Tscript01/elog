<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs transition-colors dark:border-slate-800 dark:bg-slate-900">
          ...
          <div class="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                id="company-contact"
                v-model.trim="form.company_contact"
                type="tel"
                autocomplete="tel"
                placeholder="+234..."
                :disabled="inputsDisabled"
            ...
            <!-- Supervisor Name -->
            <div>
              <label for="supervisor-name" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Industry Supervisor Full Name
              </label>
              <input
                id="supervisor-name"
                v-model.trim="form.ind_supervisor_name"
                type="text"
                placeholder="Engr. / Dr. / Mr. Name"
                :disabled="inputsDisabled"
                class="mt-1.5 block w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-2xs transition focus:border-slate-900 focus:outline-hidden disabled:bg-slate-100 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:disabled:bg-slate-800/50 dark:disabled:text-slate-400"
              />
            </div>
            ...
            <div>
              ...
              <input
                id="supervisor-email"
                v-model.trim="form.ind_supervisor_email"
                type="email"
                required
                ...
                class="mt-1.5 block w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-2xs transition focus:border-slate-900 focus:outline-hidden disabled:bg-slate-100 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:disabled:bg-slate-800/50 dark:disabled:text-slate-400"
              />
              <p v-if="errors.ind_supervisor_email" class="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">
                {{ errors.ind_supervisor_email }}
              </p>
            </div>
      ...
      <!-- Right Column: Analytics & Reviewers -->
      <div class="space-y-6">
        <!-- Duration Card -->
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs transition-colors dark:border-slate-800 dark:bg-slate-900">
          ...
          <div class="mt-4 rounded-xl bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
            SIWES placements require a minimum duration of 1 month (30 days). Standard institutional rotations typically run for 24 weeks.
          </div>
        </div>
          ...
          <ul class="mt-4 space-y-4 text-xs">
            <li class="rounded-xl border border-slate-100 bg-slate-50/50 p-3 dark:border-slate-800 dark:bg-slate-800/40">
              ...
              <p class="mt-1 font-bold text-slate-800 dark:text-slate-100">
                {{ assignedSupervisor.name || form.ind_supervisor_name || 'Pending Verification' }}
              </p>
              <p class="mt-0.5 truncate text-slate-500 dark:text-slate-400">
                {{ assignedSupervisor.email || form.ind_supervisor_email || 'No email registered' }}
              </p>
            </li>
            ...
            <li class="rounded-xl border border-slate-100 bg-slate-50/50 p-3 dark:border-slate-800 dark:bg-slate-800/40">
              ...
              <p class="mt-0.5 truncate text-slate-500 dark:text-slate-400">
                {{ assignedCoordinator.email || 'Awaiting coordinator allocation' }}
              </p>
            </li>
          </ul>
        </div>
...
watch(
  ...
  (newPlacement) => {
    if (!isEditing.value) {
      syncFormFromStore(newPlacement)
    }
  },
  { deep: true }
)
...
const cancelOrReset = () => {
  ...
  if (hasExistingPlacement.value) {
    ...
    form.ind_supervisor_name = ''
    form.ind_supervisor_email = ''
    form.start_date = ''
    form.end_date = ''
...
const save = async () => {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.ind_supervisor_email)) {
    nextErrors.ind_supervisor_email = 'Provide a valid industry supervisor email address.'
  }
  ...
  if (!form.start_date || !form.end_date) {
    ...
    } else if (endMs - startMs < thirtyDaysMs) {
      nextErrors.end_date = 'SIWES industrial training duration must be at least 1 month (30 days).'
    }
  }
  ...
  if (Object.keys(nextErrors).length > 0) {
    toast.error(new Error('Please correct the validation errors in the form.'), 'Validation Incomplete')
    return
  }
  ...
  try {
    const updated = await placementStore.savePlacement({
      ...
      ind_supervisor_email: form.ind_supervisor_email.trim(),
      supervisor_email: form.ind_supervisor_email.trim(),
    ...
    toast.success(
      'Placement Saved',
      hasExistingPlacement.value
        ? 'Placement details updated successfully.'
        : 'Placement record registered successfully.'
    )

    isEditing.value = false
    syncFormFromStore(updated || placementStore.placement)
  } catch (err: unknown) {
    toast.error(err, 'Failed to Save Placement')
  }
}
